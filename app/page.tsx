...
export default function Home() {
  return (
    <>
      ...
      <section id='contato' className='bg-slate-950 py-10 text-white'><div className='mx-auto max-w-4xl px-5 text-center'><span className='text-xs font-semibold uppercase tracking-[0.18em] text-blue-400'>Entre em contato</span><h2 className='mt-2 text-3xl font-bold tracking-tight lg:text-4xl'>Vamos transformar a educação juntos?</h2><p className='mx-auto mt-3 max-w-2xl text-sm leading-6 text-slate-300'>Fale com a InovMakers para conhecer nossos projetos, formações e soluções em educação, tecnologia e inovação.</p><div className='mx-auto mt-6 grid max-w-3xl gap-3 sm:grid-cols-2'><a href='mailto:inovmaker@gmail.com' className='group flex items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900/80 p-4 text-left transition hover:border-blue-500 hover:bg-slate-900'><div className='flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-blue-600 text-lg font-bold'>@</div><div className='min-w-0'><span className='block text-[10px] font-bold uppercase tracking-wider text-blue-400'>E-mail</span><strong className='block text-base font-bold text-white'>inovmaker@gmail.com</strong><span className='block text-xs text-slate-400'>Fale diretamente com a InovMakers</span></div></a><a href='https://www.instagram.com/inovmakers' target='_blank' rel='noopener noreferrer' className='group flex items-center gap-3 rounded-2xl border border-slate-700 bg-slate-900/80 p-4 text-left transition hover:border-pink-500 hover:bg-slate-900'><div className='flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-purple-600 via-pink-500 to-orange-400'><svg viewBox='0 0 24 24' className='h-6 w-6 fill-none stroke-white' strokeWidth='2'><rect x='3' y='3' width='18' height='18' rx='5'/><circle cx='12' cy='12' r='4'/><circle cx='17.5' cy='6.5' r='1' className='fill-white stroke-none'/></svg></div><div className='min-w-0'><span className='block text-[10px] font-bold uppercase tracking-wider text-pink-400'>Instagram</span><strong className='block text-base font-bold text-white'>@inovmakers</strong><span className='block text-xs text-slate-400'>Projetos, novidades e iniciativas</span></div></a></div><div className='mx-auto mt-5 max-w-3xl border-t border-slate-800 pt-4'><p className='text-xs text-slate-400'>InovMakers | Educação, Tecnologia e Inovação</p><p className='mt-1 text-xs text-slate-500'>Conectando pessoas, conhecimento, criatividade e tecnologia para transformar a aprendizagem.</p></div></div></section>
</>
  );
}
